...
> **AutoClicker Suite не содержит вирусов, троянов или вредоносного кода.**

Некоторые антивирусы могут пометить программу как подозрительную. Это **ложное срабатывание (false positive)**, и вот почему оно происходит:

| Причина | Пояснение |
|---------|-----------|
| **Эмуляция ввода** | Программа вызывает Win32 API-функции `SendInput`, `mouse_event`, `keybd_event` для имитации кликов и нажатий клавиш. Эти же API используют кейлоггеры, поэтому антивирусы реагируют на их присутствие в коде |
| **Глобальные хуки** | Регистрация глобальных горячих клавиш через `RegisterHotKey` и низкоуровневые хуки клавиатуры (`SetWindowsHookEx`) — поведение, характерное для шпионского ПО, хотя здесь оно используется исключительно для управления автокликером |
| **Работа с чужими окнами** | Функции `FindWindow`, `EnumWindows`, `SetForegroundWindow` для привязки к окну выглядят как поведение вредоносного ПО, автоматизирующего другие приложения |
| **Отсутствие цифровой подписи** | Исполняемый файл не подписан сертификатом (Code Signing стоит ~$200+/год), что автоматически повышает уровень недоверия у эвристических движков |

### Доказательства безопасности

- **Открытый исходный код** — весь код проекта доступен в этом репозитории. Вы можете самостоятельно проверить каждую строку и собрать программу из исходников
- **Проверка на VirusTotal** — загрузите файл на [VirusTotal](https://www.virustotal.com/) и увидите, что подавляющее большинство движков (60+) считают файл безопасным; 1–3 детекта — это типичная картина для любого автокликера
- **Сборка из исходников** — вы можете клонировать репозиторий и собрать `.exe` самостоятельно (см. раздел «Сборка из исходников» ниже), чтобы убедиться, что бинарник соответствует исходному коду
- **Никаких сетевых подключений** — программа не обращается к интернету, не отправляет данные и не скачивает ничего (можно проверить в любом сетевом мониторе, например Wireshark)

> **Если антивирус блокирует запуск:** добавьте `AutoClickerSuite.exe` в список исключений вашего антивируса.
